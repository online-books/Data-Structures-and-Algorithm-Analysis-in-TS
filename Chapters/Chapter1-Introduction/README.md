...
定义：X^A=B,当且仅当$\log_XB=A$(<font size=2>除非特别说明，所有的对数都是以 2 为底的</font>)。
...
为了求解方程 2-1，我们在方程两边同时除以$N$，得到
$$ \frac{T(N)}{N}=\frac{T(N/2)}{N/2}+1 \tag{2-3} $$

将$\frac{N}{2}$代入方程 2-3 中，得到
$$\frac{T(N/2)}{N/2}=\frac{T(N/4)}{N/4}+1 \tag{2-4}$$
$$\vdots$$
$$\frac{T(2)}{2}=T(1)+1$$

从$T(N)$到$T(2)$，我们一共递归地代入了$\log N$次，将所有这些方程相加后，得到

$$ \frac{T(N)}{N}=T(1)+ \log N \tag{2-5} $$

方程 2-5 两边同乘$N$后，得到

$$ T(N)=N+ N\log N \tag{2-5} $$
